...
if not st.session_state.authenticated:
    ...
    st.write("This engine is locked. Enter your assigned email and password to begin.")
...
if show_admin_panel and st.session_state.is_admin:
    ...
    adm_tab1, adm_tab2, adm_tab3 = st.tabs(["👥 User Management", "📊 Activity Logs", "⚙️ System Configuration"])
    with adm_tab3:
        sys_config = get_system_config()
        override_switch = st.checkbox("⚠️ Activate Global Speed Override", value=sys_config["override_global_speed"])
        global_speed_slider = st.number_input("Default Global Speed Limit (ms):", min_value=0.01, max_value=2000.0, value=float(sys_config["throttle_delay_ms"]), step=0.1, format="%.2f")
...
if not show_admin_panel:
    ...
    st.write("Control the cloud background crawler. Once started, it runs independently on the server 24/7 even if you shut down your PC.")
    ...
    with col_st1:
        if is_running:
            st.success(f"🟢 Cloud Crawler is ACTIVE (Currently processing MC-{current_mc_val})")
        else:
            st.warning("🔴 Cloud Crawler is STOPPED")
    with col_st2:
        st.metric("Enforced Speed Limit", speed_mode_string)

    new_mc_input = st.text_input("Set / Reset Starting MC Number:", value=str(current_mc_val))
    ...
    if col_btn1.button("🚀 Start 24/7 Cloud Sequence", use_container_width=True):
        if new_mc_input.isdigit():
            ...
            log_activity(st.session_state.current_user, "start_cloud_crawler", f"Resumed/Started at MC-{new_mc_input}")
    ...
    if col_btn2.button("🛑 STOP Cloud Sequence", use_container_width=True):
        ...
        log_activity(st.session_state.current_user, "stop_cloud_crawler", f"Paused at MC-{current_mc_val}")
